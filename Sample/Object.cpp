...
CGameObject::CGameObject(int nResource)
{
	m_xmf4x4Transform = Matrix4x4::Identity();
	m_xmf4x4World = Matrix4x4::Identity();

	m_nMeshInHierarchy = nResource;
}
...
void CGameObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, ID3D12Resource** ppd3dcbGameObjects, UINT8** ppcbMappedGameObjects)
{
	OnPrepareRender();

	if (m_pMesh)
	{
		UINT ncbGameObjectBytes = ((sizeof(XMFLOAT4X4) + (sizeof(XMFLOAT4X4) * MATERIALS_IN_HIERARCHY) + 255) & ~255); //256의 배수

		D3D12_GPU_VIRTUAL_ADDRESS d3dcbGameObjectGpuVirtualAddress = ppd3dcbGameObjects[m_nMeshInHierarchy]->GetGPUVirtualAddress();
		pd3dCommandList->SetGraphicsRootConstantBufferView(1, d3dcbGameObjectGpuVirtualAddress);

		UINT8* pbMappedcbGameObject = ppcbMappedGameObjects[m_nMeshInHierarchy];
		UpdateShaderVariables(pd3dCommandList, pbMappedcbGameObject);

		if (m_nMaterials > 0)
		{
			for (int i = 0; i < m_nMaterials; i++)
			{
				if (m_ppMaterials[i] && m_ppMaterials[i]->m_pShader) m_ppMaterials[i]->m_pShader->Render(pd3dCommandList, pCamera);
				pd3dCommandList->SetGraphicsRoot32BitConstant(3, i, 0); //gnMaterial
				if (m_pMesh) m_pMesh->Render(pd3dCommandList, i);
			}
		}
	}

	if (m_pSibling) m_pSibling->Render(pd3dCommandList, pCamera, ppd3dcbGameObjects, ppcbMappedGameObjects);
	if (m_pChild) m_pChild->Render(pd3dCommandList, pCamera, ppd3dcbGameObjects, ppcbMappedGameObjects);
}

void CGameObject::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int nMeshesInHierarchy, int *pnMaterialsInHierarchy)
{
	m_nMeshesInHierarchy = nMeshesInHierarchy; 
	m_ppd3dcbInstancingGameObjects = new ID3D12Resource*[nMeshesInHierarchy];
	m_ppcbMappedInstancingGameObjects = new UINT8*[nMeshesInHierarchy];
	m_pnMaterialsInHierarchy = new int[nMeshesInHierarchy];

	for (int i = 0; i < m_nMeshesInHierarchy; i++)
	{
		m_pnMaterialsInHierarchy[i] = pnMaterialsInHierarchy[i];
		UINT ncbGameObjectBytes = ((sizeof(XMFLOAT4X4) + (sizeof(XMFLOAT4X4) * MATERIALS_IN_HIERARCHY) + 255) & ~255); //256의 배수
		m_ppd3dcbInstancingGameObjects[i] = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbGameObjectBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);

		m_ppd3dcbInstancingGameObjects[i]->Map(0, NULL, (void**)&m_ppcbMappedInstancingGameObjects[i]);
	}
}

void CGameObject::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList, UINT8* pbMappedcbGameObject)
{
	if (pbMappedcbGameObject)
	{
		XMFLOAT4X4 xmf4x4World;
		XMStoreFloat4x4(&xmf4x4World, XMMatrixTranspose(XMLoadFloat4x4(&m_xmf4x4World)));
		::memcpy(pbMappedcbGameObject, &xmf4x4World, sizeof(XMFLOAT4X4));

		if (m_nMaterials > 0)
		{
			pbMappedcbGameObject = pbMappedcbGameObject + sizeof(XMFLOAT4X4);
			for (int i = 0; i < m_nMaterials; i++)
			{
				::memcpy(pbMappedcbGameObject, &m_ppMaterials[i]->m_pMaterialColors->m_xmf4Ambient, sizeof(XMFLOAT4));
				::memcpy(pbMappedcbGameObject + sizeof(XMFLOAT4), &m_ppMaterials[i]->m_pMaterialColors->m_xmf4Diffuse, sizeof(XMFLOAT4));
				::memcpy(pbMappedcbGameObject + sizeof(XMFLOAT4) * 2, &m_ppMaterials[i]->m_pMaterialColors->m_xmf4Emissive, sizeof(XMFLOAT4));
				::memcpy(pbMappedcbGameObject + sizeof(XMFLOAT4) * 3, &m_ppMaterials[i]->m_pMaterialColors->m_xmf4Specular, sizeof(XMFLOAT4));
				pbMappedcbGameObject = pbMappedcbGameObject + sizeof(XMFLOAT4X4);
			}
		}
	}
...
{
	if (m_ppd3dcbInstancingGameObjects)
	{
		for (int i = 0; i < m_nMeshesInHierarchy; i++)
		{
			m_ppd3dcbInstancingGameObjects[i]->Unmap(0, NULL);
			m_ppd3dcbInstancingGameObjects[i]->Release();
			m_ppd3dcbInstancingGameObjects[i] = NULL;
		}
		delete[] m_ppd3dcbInstancingGameObjects;
		m_ppd3dcbInstancingGameObjects = NULL;
	}
	if (m_ppcbMappedInstancingGameObjects) delete[] m_ppcbMappedInstancingGameObjects;
	if (m_pnMaterialsInHierarchy) delete[] m_pnMaterialsInHierarchy;
...
CGameObject *CGameObject::LoadFrameHierarchyFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, FILE *pInFile, int* pnMeshesInHierarchy, int* pnMaterialsInHierarchy)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
	...
	for ( ; ; )
	{
		...
		{
			pGameObject->m_nMeshInHierarchy = (*pnMeshesInHierarchy)++;
		...
		else if (!strcmp(pstrToken, "<Materials>:"))
		{
			...
			if (pMaterialsInfo && (pMaterialsInfo->m_nMaterials > 0))
			{
				...
				pnMaterialsInHierarchy[pGameObject->m_nMeshInHierarchy] = pMaterialsInfo->m_nMaterials;
		...
		else if (!strcmp(pstrToken, "<Children>:"))
		{
			...
			if (nChilds > 0)
			{
				...
				{
					CGameObject *pChild = CGameObject::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pInFile, pnMeshesInHierarchy, pnMaterialsInHierarchy);
...
CGameObject *CGameObject::LoadGeometryFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, char *pstrFileName, int *pnMeshesInHierarchy, int* pnMaterialsInHierarchy)
{
	...
	for ( ; ; )
	{
		...
		{
			pGameObject = CGameObject::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pInFile, pnMeshesInHierarchy, pnMaterialsInHierarchy);
		}
...
void CHellicopterObject::Animate(float fTimeElapsed, XMFLOAT4X4 *pxmf4x4Parent)
{
	...
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationX(XMConvertToRadians(360.0f * 0.1f) * fTimeElapsed);
...
void CTankObject::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	if (m_pTurretFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationY(XMConvertToRadians(0.0f) * fTimeElapsed);
		m_pTurretFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_pTurretFrame->m_xmf4x4Transform);
	}
	if (m_pCannonFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationX(
			XMConvertToRadians(30.0f) * fTimeElapsed  // X축 상하
		);
		m_pCannonFrame->m_xmf4x4Transform = Matrix4x4::Multiply(
			xmmtxRotate, m_pCannonFrame->m_xmf4x4Transform
		);
	}
	CGameObject::Animate(fTimeElapsed, pxmf4x4Parent);
}
