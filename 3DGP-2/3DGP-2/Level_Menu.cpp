                              ...
                              ID3D12RootSignature* pd3dRootSignature)
{
    ...
    BuildLabel(pd3dDevice, pd3dCommandList, "TUTORIAL", ITEM_TUTORIAL, s_ItemNdcY[0] * kWorldPerNdcY, white);
    BuildLabel(pd3dDevice, pd3dCommandList, "LEVEL-1",  ITEM_LEVEL1,   s_ItemNdcY[1] * kWorldPerNdcY, white);
    BuildLabel(pd3dDevice, pd3dCommandList, "LEVEL-2",  ITEM_LEVEL2,   s_ItemNdcY[2] * kWorldPerNdcY, white);
    BuildLabel(pd3dDevice, pd3dCommandList, "LEVEL-3",  ITEM_LEVEL3,   s_ItemNdcY[3] * kWorldPerNdcY, white);
    BuildLabel(pd3dDevice, pd3dCommandList, "START",    ITEM_START,    s_ItemNdcY[4] * kWorldPerNdcY, green);
    BuildLabel(pd3dDevice, pd3dCommandList, "END",      ITEM_END,      s_ItemNdcY[5] * kWorldPerNdcY, red);

    UpdateTransforms();
}

void CLevel_Menu::BuildLabel(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
                             const char* text, int item, float centerY, XMFLOAT4 color)
{
    m_ItemCenterY[item] = centerY;

    float width = MeasureAscii(text, CELL, SPACING);
    float penX  = -width * 0.5f;

    for (const char* p = text; *p; ++p)
    {
        std::vector<XMFLOAT3> cells;
        float advance = LayoutGlyph(*AsciiGlyph(*p), penX, centerY, CELL, SPACING, cells);
        ...
        for (const XMFLOAT3& c : cells)
        {
            ...
            MenuCube mc;
            mc.pObj = pCube;
            mc.rel  = XMFLOAT3(c.x, c.y - centerY, 0.0f);  // relative to item center
            mc.item = item;
...
void CLevel_Menu::UpdateTransforms()
{
    ...
    for (MenuCube& mc : m_Cubes)
    {
        ...
        float s   = sel ? 1.30f : 1.0f;
        float z   = sel ? -0.30f : 0.0f;  // pop selected item toward camera

        XMMATRIX world = XMMatrixScaling(s, s, s) *
                         XMMatrixTranslation(mc.rel.x * s,
                                             m_ItemCenterY[mc.item] + mc.rel.y * s,
                                             mc.rel.z * s + z);
...
int CLevel_Menu::Update(float dt)
{
    ...
    if (pInput->Key_Down(VK_LBUTTON))
    {
        ...
        case ITEM_LEVEL2:
        case ITEM_LEVEL3:
            m_nSelectedStage = hit;        // item index == stage for 0..3
            UpdateTransforms();
            break;
        case ITEM_START:
            CLevel_Manager::Get_Instance()->Request_Level_Change(LEVEL_GAMEPLAY, m_nSelectedStage);
            break;
        case ITEM_END:
            PostQuitMessage(0);
            break;
...
{
    for (MenuCube& mc : m_Cubes)
        if (mc.pObj) { delete mc.pObj; mc.pObj = nullptr; }
    m_Cubes.clear();
