...
{
    int selectedItem = m_nSelectedStage;
    ...
    for(MenuCube& mc : m_Cubes)
    {
        ...
        if(sel)
            mc.pObj->SetMesh(m_pBlueMesh);
        else
            mc.pObj->SetMesh(m_ItemMesh[mc.item]);
    }
}

int CLevel_Menu::HitTest(float ndcX, float ndcY) const
{
    if(ndcX < HIT_X_MIN || ndcX > HIT_X_MAX)
        return -1;
...
int CLevel_Menu::Update(float dt)
{
    if(CInput_Manager::Get_Instance()->Key_Down(VK_LBUTTON))
    {
        ...
        case ITEM_LEVEL3:
            ...
            break;
        case ITEM_START:
            CLevel_Manager::Get_Instance()->Request_Level_Change(LEVEL_STAGE1);
            break;
        case ITEM_END:
            PostQuitMessage(0);
            break;
...
void CLevel_Menu::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
    ...
    for(MenuCube& mc : m_Cubes)
        if(mc.pObj)
            mc.pObj->Render(pd3dCommandList, m_pCamera);
}
...
void CLevel_Menu::ReleaseUploadBuffers()
{
    for(int i = 0; i < ITEM_COUNT; ++i)
        if(m_ItemMesh[i])
            m_ItemMesh[i]->ReleaseUploadBuffers();
    if(m_pBlueMesh)
        m_pBlueMesh->ReleaseUploadBuffers();
}
...
void CLevel_Menu::Release()
{
    ...
    for(int i = 0; i < ITEM_COUNT; ++i)
        if(m_ItemMesh[i])
        {
            m_ItemMesh[i]->Release();
            m_ItemMesh[i] = nullptr;
        }
    if(m_pBlueMesh)
    {
        m_pBlueMesh->Release();
