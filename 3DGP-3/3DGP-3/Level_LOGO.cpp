...
int CLevel_LOGO::Update(float dt)
{
    ...
    if (CInput_Manager::Get_Instance()->Key_Down(VK_LBUTTON))
    {
            ...
            ndcY >= NAME_NDC_Y_MIN && ndcY <= NAME_NDC_Y_MAX)
        {
            ...
            pFx->Init(m_pd3dDevice, m_pShader, XMFLOAT3(0.0f, m_fNameY, 0.0f));
...
void CLevel_LOGO::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
    ...
    for (LogoCube& lc : m_TitleCubes) if (lc.pObj) lc.pObj->Render(pd3dCommandList, m_pCamera);
    if (!m_bExploding)
        for (LogoCube& lc : m_NameCubes) if (lc.pObj) lc.pObj->Render(pd3dCommandList, m_pCamera);
...
{
    for (LogoCube& lc : m_TitleCubes) if (lc.pObj) lc.pObj->ReleaseUploadBuffers();
    for (LogoCube& lc : m_NameCubes)  if (lc.pObj) lc.pObj->ReleaseUploadBuffers();
...
void CLevel_LOGO::Release()
{
    ...
    for (LogoCube& lc : m_TitleCubes) if (lc.pObj) lc.pObj->Release();
    for (LogoCube& lc : m_NameCubes)  if (lc.pObj) lc.pObj->Release();
    m_TitleCubes.clear();
    m_NameCubes.clear();
