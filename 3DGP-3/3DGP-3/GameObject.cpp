...
CGameObject::CGameObject()
{
    ...
    m_xmOOBB.Extents = XMFLOAT3(1.0f, 1.0f, 1.0f);
...
CGameObject* CGameObject::FindFrame(char* pstrFrameName)
{
    ...
    if(lenA > lenB)
        ...
        maxLen = lenB;
    if(!strncmp(m_pstrFrameName, pstrFrameName, maxLen))
        return this;

    if(m_pSibling)
        if((pFrame = m_pSibling->FindFrame(pstrFrameName)))
            return pFrame;
    if(m_pChild)
        if((pFrame = m_pChild->FindFrame(pstrFrameName)))
            return pFrame;
    return NULL;
}
...
void CGameObject::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
    ...
    int nMats;
    if(m_nMaterials < MATERIALS_IN_HIERARCHY)
        nMats = m_nMaterials;
    else
        nMats = MATERIALS_IN_HIERARCHY;
    for(int i = 0; i < nMats; i++)
    {
        CMaterialColors* pColors;
        if(m_ppMaterials[i])
            pColors = m_ppMaterials[i]->m_pMaterialColors;
        else
            pColors = NULL;
        if(pColors)
        {
...
void CGameObject::ReleaseShaderVariables()
{
    ...
    if(m_pSibling)
        m_pSibling->ReleaseShaderVariables();
    if(m_pChild)
        m_pChild->ReleaseShaderVariables();
}
    ...
    const char* pstrFileName, CShader* pShader)
{
    ...
    for(;;)
    {
        if(ReadStringFromFile(pInFile, pstrToken) == 0)
            break;
